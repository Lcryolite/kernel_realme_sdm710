...
static ulong ramoops_device_info_size;
module_param_named(device_info_size, ramoops_device_info_size, ulong, 0400);
MODULE_PARM_DESC(device_info_size,
		"size reserved for bootloader-visible device information");
			     ...
			     unsigned int *cnt, u32 sig, u32 flags)
{
	...
	if (record_size < 0) {
		...
		*cnt = mem_sz / record_size;
		...
		/*
		 * Keep explicitly sized zones at exactly record_size.  RMX1901
		 * firmware consumes the unused tail and locates the console from
		 * this fixed layout; redistributing it moves the console address.
		 */
		zone_sz = record_size;
	}
