...
record_dtb_fingerprint() {
  ...
  # kernel_dtb.  Keep the validated group already installed in boot.  Recovery
  ...
  if [ -f "$SPLITIMG/kernel_dtb" ]; then
    ...
    [ "$RMX1901_DTB_SHA256" ] || abort "The expected RMX1901 DTB hash is missing. Aborting...";
    current_hash=$(ak3_hash "$SPLITIMG/kernel_dtb") || \
      abort "Unable to hash the boot DTB group. Aborting...";
    [ "$current_hash" = "$RMX1901_DTB_SHA256" ] || \
      abort "Boot DTB hash differs from the validated ODM-fixed group. Aborting...";

    printf 'source=boot\nruntime_boot_device=%s\ndtb_count=%s\ndtb_size=%s\ndtb_sha256=%s\n' \
      "$runtime_boot_device" "$current_count" "$(wc -c < "$SPLITIMG/kernel_dtb" | tr -d '[:space:]')" "$current_hash" \
      > "$DTB_VERIFY_DIR/source.manifest";
    ui_print " " "Preserving validated boot DTB group for $runtime_boot_device";
  fi;
