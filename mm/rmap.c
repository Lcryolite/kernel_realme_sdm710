...
int anon_vma_clone_oppo(struct vm_area_struct *dst, struct vm_area_struct *src)
{
	...
	list_for_each_entry_reverse(pavc, &src->anon_vma_chain, same_vma) {
		...
		/*
		 * Reuse existing anon_vma if it has no vmas and only one
		 * anon_vma child. This prevents an endless hierarchy chain.
		 *
 ...
 enomem_failure:
	/*
	 * dst->anon_vma is dropped here otherwise its active-vma count can be incorrectly
	 * decremented in unlink_anon_vmas().
	 * We can safely do this because callers of anon_vma_clone() don't care
	 * about dst->anon_vma if anon_vma_clone() failed.
	 */
