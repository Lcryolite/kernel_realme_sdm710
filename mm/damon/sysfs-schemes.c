		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	struct damon_sysfs_scheme_filter *filter = container_of(kobj,
			struct damon_sysfs_scheme_filter, kobj);
	char *path = kmalloc_array(count + 1, sizeof(*path),
				   GFP_KERNEL);
...
static struct kobj_attribute damon_sysfs_scheme_filters_nr_attr =
		__ATTR(nr_filters, 0600, nr_filters_show, nr_filters_store);
...
static struct kobj_attribute damos_sysfs_quota_goal_target_metric_attr =
		__ATTR(target_metric, 0600, target_metric_show, target_metric_store);

static struct kobj_attribute damos_sysfs_quota_goal_target_value_attr =
		__ATTR(target_value, 0600, target_value_show, target_value_store);

static struct kobj_attribute damos_sysfs_quota_goal_current_value_attr =
		__ATTR(current_value, 0600, current_value_show, current_value_store);

static struct kobj_attribute damos_sysfs_quota_goal_nid_attr =
		__ATTR(nid, 0600, nid_show, nid_store);
...
static struct kobj_attribute damos_sysfs_quota_goals_nr_attr =
		__ATTR(nr_goals, 0600, nr_goals_show, nr_goals_store);
...
static struct kobj_attribute damon_sysfs_weights_sz_attr =
		__ATTR(sz_permil, 0600, sz_permil_show, sz_permil_store);

static struct kobj_attribute damon_sysfs_weights_nr_accesses_attr =
		__ATTR(nr_accesses_permil, 0600, nr_accesses_permil_show, nr_accesses_permil_store);

static struct kobj_attribute damon_sysfs_weights_age_attr =
		__ATTR(age_permil, 0600, age_permil_show, age_permil_store);
...
static struct kobj_attribute damon_sysfs_quotas_ms_attr =
		__ATTR(ms, 0600, ms_show, ms_store);

static struct kobj_attribute damon_sysfs_quotas_sz_attr =
		__ATTR(bytes, 0600, bytes_show, bytes_store);

static struct kobj_attribute damon_sysfs_quotas_reset_interval_ms_attr =
		__ATTR(reset_interval_ms, 0600, reset_interval_ms_show, reset_interval_ms_store);
...
static struct kobj_attribute damos_sysfs_dest_id_attr =
		__ATTR(id, 0600, id_show, id_store);

static struct kobj_attribute damos_sysfs_dest_weight_attr =
		__ATTR(weight, 0600, weight_show, weight_store);
...
static const struct kobj_type damos_sysfs_dest_ktype = {
	...
	.sysfs_ops = &kobj_sysfs_ops,
	.default_attrs = damos_sysfs_dest_attrs,
};
...
static struct kobj_attribute damos_sysfs_dests_nr_attr =
		__ATTR(nr_dests, 0600, nr_dests_show, nr_dests_store);
...
static const struct kobj_type damos_sysfs_dests_ktype = {
	...
	.sysfs_ops = &kobj_sysfs_ops,
	.default_attrs = damos_sysfs_dests_attrs,
};
...
static struct kobj_attribute damon_sysfs_scheme_action_attr =
		__ATTR(action, 0600, action_show, action_store);

static struct kobj_attribute damon_sysfs_scheme_apply_interval_us_attr =
		__ATTR(apply_interval_us, 0600, apply_interval_us_show, apply_interval_us_store);

static struct kobj_attribute damon_sysfs_scheme_target_nid_attr =
		__ATTR(target_nid, 0600, target_nid_show, target_nid_store);
...
static struct kobj_attribute damon_sysfs_schemes_nr_attr =
		__ATTR(nr_schemes, 0600, nr_schemes_show, nr_schemes_store);
