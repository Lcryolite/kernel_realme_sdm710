	...
	*type = BPF_TYPE_MAP;
	...
	if (IS_ERR(raw)) {
		*type = BPF_TYPE_PROG;
		...
		if (IS_ERR(raw)) {
			*type = BPF_TYPE_RAW_TRACEPOINT;
			raw = bpf_raw_tracepoint_get_file(ufd);
		}
	}
...
static const struct inode_operations bpf_raw_tracepoint_iops = { };
...
static int bpf_inode_type(const struct inode *inode, enum bpf_type *type)
{
	*type = BPF_TYPE_UNSPEC;
	if (inode->i_op == &bpf_prog_iops)
		...
		*type = BPF_TYPE_MAP;
	else if (inode->i_op == &bpf_raw_tracepoint_iops)
		*type = BPF_TYPE_RAW_TRACEPOINT;
	else
		return -EACCES;
		     ...
		     dev_t devt)
{
	...
	case BPF_TYPE_RAW_TRACEPOINT:
		return bpf_mkobj_ops(dir, dentry, mode,
				       &bpf_raw_tracepoint_iops, NULL);
	default:
		return -EPERM;
	}
}
...
static int bpf_raw_tracepoint_new_fd(struct file *file)
{
	int fd;
...
int bpf_obj_get_user(const char __user *pathname, int flags)
{
	...
	else if (type == BPF_TYPE_RAW_TRACEPOINT)
		ret = bpf_raw_tracepoint_new_fd(raw);
	else
		goto out;
