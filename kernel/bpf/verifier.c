...
static bool rmxdiag_parse_btf;

module_param(rmxdiag_parse_btf, bool, 0644);
MODULE_PARM_DESC(rmxdiag_parse_btf,
		 "Parse built-in BTF on the next BPF program load for diagnosis");
	...
	for (i = 0; i < insn_cnt; i++) {
		if (bpf_pseudo_func(insn + i)) {
			if (!env->allow_ptr_leaks) {
				verbose(env, "function pointers are allowed for root only\n");
				return -EPERM;
			}
...
{
	struct bpf_map *map = cur_regs(env)[regno].map_ptr;

	if (type == BPF_WRITE && (map->map_flags & BPF_F_RDONLY_PROG)) {
			...
			struct bpf_insn *insn)
{
	...
	/* Check whether it can write into the same memory. */
				   ...
				   struct bpf_call_arg_meta *meta)
{
	...
	case PTR_TO_MEM:
		return check_mem_region_access(env, regno, reg->off,
					       ...
					       zero_size_allowed);
	case PTR_TO_BTF_ID:
		if (!meta || !meta->readonly_btf_mem) {
			verbose(env, "R%d kernel BTF memory is not allowed for this helper\n",
				regno);
			return -EACCES;
		}
			  ...
			  struct bpf_call_arg_meta *meta)
{
	...
	if (arg_type == ARG_PTR_TO_MAP_KEY ||
		...
		if (register_is_null(reg) &&
			...
			/* final test in check_stack_boundary() */;
			 ...
			 type != PTR_TO_MAP_VALUE &&
			 type != PTR_TO_MEM &&
			 !(arg_type == ARG_PTR_TO_RDONLY_MEM &&
			   type == PTR_TO_BTF_ID) &&
			 type != expected_type)
			goto err_type;
		meta->raw_mode = arg_type == ARG_PTR_TO_UNINIT_MEM;
		meta->readonly_btf_mem = arg_type == ARG_PTR_TO_RDONLY_MEM;
	...
	if (arg_type == ARG_CONST_MAP_PTR) {
		...
		if (!tnum_is_const(reg->var_off)) {
			verbose(env, "R%d is not a constant string address\n", regno);
			return -EACCES;
		}
		if (reg->off < 0 || reg->var_off.value > U32_MAX) {
			verbose(env, "R%d has an invalid string offset\n", regno);
			return -EACCES;
		}

		map_off = reg->off + reg->var_off.value;
		if (map_off >= map->value_size) {
			verbose(env, "R%d string offset is outside the map value\n", regno);
			return -EACCES;
		}
		if (!map->ops->map_direct_value_addr) {
			verbose(env, "map type has no direct string access\n");
			return -EACCES;
		}
		...
		/* Linux 4.9 strnchr() cannot match the terminating NUL. */
		if (!memchr(str_ptr + map_off, 0, map->value_size - map_off)) {
...
static int check_map_elem_callback_call(struct bpf_verifier_env *env,
					int *insn_idx, u32 subprogno,
					const struct bpf_reg_state *map_reg,
					const struct bpf_reg_state *ctx_reg)
{
	struct bpf_verifier_state *state = env->cur_state;
	struct bpf_func_state *callee;
	struct bpf_insn callback_call = {
		.code = BPF_JMP | BPF_CALL,
		.src_reg = BPF_PSEUDO_CALL,
	};
	int err;

	if (subprogno >= env->subprog_cnt)
		return -EINVAL;
	if (!map_reg->map_ptr->ops->map_set_for_each_callback_args ||
	    !map_reg->map_ptr->ops->map_for_each_callback) {
		verbose(env, "callback function not allowed for map\n");
		return -ENOTSUPP;
	}

	callback_call.imm = env->subprog_info[subprogno].start - *insn_idx - 1;
	err = check_func_call(env, &callback_call, insn_idx);
	if (err)
		return err;

	callee = state->frame[state->curframe];
	callee->regs[BPF_REG_1] = *map_reg;
	callee->regs[BPF_REG_2].type = PTR_TO_MAP_KEY;
	__mark_reg_known_zero(&callee->regs[BPF_REG_2]);
	callee->regs[BPF_REG_2].map_ptr = map_reg->map_ptr;
	callee->regs[BPF_REG_3].type = PTR_TO_MAP_VALUE;
	__mark_reg_known_zero(&callee->regs[BPF_REG_3]);
	callee->regs[BPF_REG_3].map_ptr = map_reg->map_ptr;
	callee->regs[BPF_REG_4] = *ctx_reg;
	__mark_reg_not_init(env, &callee->regs[BPF_REG_5]);
...
static int prepare_func_exit(struct bpf_verifier_env *env, int *insn_idx)
{
	...
	if (callee->in_callback_fn) {
		...
		if (r0->type != SCALAR_VALUE || !tnum_in(range, r0->var_off)) {
			verbose(env, "callback return R0 must be in [0, 1]\n");
			return -EINVAL;
		}
	} else {
			     ...
			     int *insn_idx_p)
{
	...
	struct bpf_reg_state callback_map = {}, callback_ctx = {};
	struct bpf_reg_state *regs;
	struct bpf_call_arg_meta meta;
	...
	if (func_id == BPF_FUNC_for_each_map_elem) {
		regs = cur_regs(env);
		callback_map = regs[BPF_REG_1];
		callback_ctx = regs[BPF_REG_3];
	}
	...
	} else if (fn->ret_type == RET_PTR_TO_BTF_ID) {
		u32 ret_btf_id;

		if (!fn->ret_btf_id_name) {
			verbose(env, "helper return BTF type name is missing\n");
			return -EINVAL;
		}
		ret_btf_id = btf_find_by_name_kind(btf_vmlinux,
						   fn->ret_btf_id_name,
						   BTF_KIND_STRUCT);
		if (!ret_btf_id) {
			verbose(env, "helper return BTF struct %s is missing\n",
				fn->ret_btf_id_name);
			return -EINVAL;
		}
		mark_reg_known_zero(env, regs, BPF_REG_0);
	...
	if (func_id == BPF_FUNC_for_each_map_elem)
		return check_map_elem_callback_call(env, insn_idx_p,
						    meta.subprogno,
						    &callback_map,
						    &callback_ctx);
	return 0;
...
static int check_ld_imm(struct bpf_verifier_env *env, struct bpf_insn *insn)
{
	...
	if (insn->src_reg == BPF_PSEUDO_FUNC) {
		...
		if (!env->prog->aux->func_info ||
		    subprogno >= env->prog->aux->func_info_cnt) {
			verbose(env, "missing btf func_info for callback\n");
			return -EINVAL;
		}
		regs[insn->dst_reg].type = PTR_TO_FUNC;
		regs[insn->dst_reg].subprogno = subprogno;
		return 0;
	}
		...
		if (class == BPF_ALU || class == BPF_ALU64) {
			...
			if (opcode == BPF_CALL) {
				...
				if (insn->src_reg == BPF_PSEUDO_CALL)
					...
					err = check_helper_call(env, insn->imm,
								&env->insn_idx);
				if (err)
					return err;
	...
	for (i = 0; i < insn_cnt; i++, insn++) {
		...
		if (insn[0].code == (BPF_LD | BPF_IMM | BPF_DW)) {
			...
			if (insn[0].src_reg == BPF_PSEUDO_FUNC) {
				if (insn[1].imm) {
					verbose(env, "invalid bpf pseudo func\n");
					return -EINVAL;
				}
				goto next_insn;
			}
...
static int check_attach_btf_id(struct bpf_verifier_env *env)
{
	...
	if (prog->type != BPF_PROG_TYPE_TRACING)
		return 0;
	if (!btf_vmlinux) {
		verbose(env, "built-in BTF parsing is deferred\n");
		return -EOPNOTSUPP;
	}
	      ...
	      union bpf_attr __user *uattr)
{
	...
	if (rmxdiag_parse_btf && !btf_vmlinux &&
