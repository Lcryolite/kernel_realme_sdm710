...
static void *btf_kvmalloc(size_t size, gfp_t flags)
{
	void *ptr;
...
static void *btf_kvzalloc(size_t size, gfp_t flags)
{
	void *ptr;

	ptr = kzalloc(size, flags | __GFP_NOWARN);
	if (!ptr)
		ptr = __vmalloc(size, flags | __GFP_ZERO, PAGE_KERNEL);
	return ptr;
}
...
u32 btf_find_by_name_kind(const struct btf *btf, const char *name, u8 kind)
{
	...
	for (id = 1; id <= btf->nr_types; id++) {
		...
		if (BTF_INFO_KIND(type->info) == kind &&
		    !strcmp(__btf_name_by_offset(btf, type->name_off), name))
			return id;
	}
...
static int btf_add_type(struct btf_verifier_env *env, struct btf_type *t)
{
	...
	if (btf->types_size - btf->nr_types < 2) {
		...
		new_types = btf_kvzalloc(new_size * sizeof(*new_types),
...
static int env_resolve_init(struct btf_verifier_env *env)
{
	...
	/* +1 for btf_void */
	resolved_sizes = btf_kvzalloc((nr_types + 1) * sizeof(*resolved_sizes),
				  GFP_KERNEL | __GFP_NOWARN);
	if (!resolved_sizes)
		goto nomem;

	resolved_ids = btf_kvzalloc((nr_types + 1) * sizeof(*resolved_ids),
				GFP_KERNEL | __GFP_NOWARN);
	if (!resolved_ids)
		goto nomem;

	visit_states = btf_kvzalloc((nr_types + 1) * sizeof(*visit_states),
				GFP_KERNEL | __GFP_NOWARN);
	if (!visit_states)
		goto nomem;
			     ...
			     u32 log_level, char __user *log_ubuf, u32 log_size)
{
	...
	data = btf_kvmalloc(btf_data_size, GFP_KERNEL | __GFP_NOWARN);
	if (!data) {
		err = -ENOMEM;
		goto errout;
	}
...
again:
	...
	for_each_member(i, t, member) {
		...
		if (btf_type_is_array(mtype)) {
			const struct btf_array *array = btf_type_array(mtype);
			const struct btf_type *elem_type;
			u32 array_type_id = member->type;
			u32 elem_type_id = array->type;
			u32 array_size;

			elem_type = btf_type_id_size(btf_vmlinux, &elem_type_id,
						     NULL);
			if (!elem_type || !btf_type_is_int(elem_type))
				continue;
			if (!btf_type_id_size(btf_vmlinux, &array_type_id,
						  &array_size))
				return -EFAULT;
			if (off < moff / 8 ||
			    off + size > moff / 8 + array_size) {
				bpf_log(log,
					"cannot access %d bytes in struct %s array field %s\n",
					size, tname, mname);
				return -EACCES;
			}
			return SCALAR_VALUE;
