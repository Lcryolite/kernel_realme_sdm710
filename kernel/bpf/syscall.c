...
static int map_create(union bpf_attr *attr)
{
	...
	if (IS_ERR(map)) {
		err = PTR_ERR(map);
		pr_info("rmxdiag: bpf_map_create reject type=%u key=%u value=%u max=%u flags=%#x ifindex=%u err=%d\n",
			attr->map_type, attr->key_size, attr->value_size,
			attr->max_entries, attr->map_flags, attr->map_ifindex,
			err);
		return err;
	}
...
static u32 bpf_map_sys_value_size(const struct bpf_map *map)
{
	if (map->map_type == BPF_MAP_TYPE_PERCPU_HASH ||
	    map->map_type == BPF_MAP_TYPE_LRU_PERCPU_HASH ||
	    map->map_type == BPF_MAP_TYPE_PERCPU_ARRAY ||
			     ...
			     union bpf_attr __user *uattr)
{
	...
	value_size = bpf_map_sys_value_size(map);
...
static int bpf_prog_load(union bpf_attr *attr, union bpf_attr __user *uattr)
{
	...
	u32 allowed_prog_flags = BPF_F_STRICT_ALIGNMENT |
		BPF_F_ANY_ALIGNMENT | BPF_F_TEST_STATE_FREQ |
		BPF_F_TEST_RND_HI32;
	struct bpf_prog *prog;
	int err;
	char license[128];
	...
	if (type == BPF_PROG_TYPE_SYSCALL)
		allowed_prog_flags |= BPF_F_SLEEPABLE;
	if (attr->prog_flags & ~allowed_prog_flags)
		return -EINVAL;
...
struct file *bpf_raw_tracepoint_get_file(u32 ufd)
{
	struct fd f = fdget(ufd);
	struct file *file;
			    ...
			    union bpf_attr __user *uattr, int cmd)
{
	...
	if (cmd == BPF_MAP_LOOKUP_AND_DELETE_BATCH &&
	    !(map_get_sys_perms(map, f) & FMODE_CAN_WRITE)) {
		err = -EPERM;
		goto err_put;
	}
	...
	if (cmd == BPF_MAP_LOOKUP_BATCH) {
		if (!map->ops->map_lookup_batch) {
			err = -ENOTSUPP;
			goto err_put;
		}
		err = map->ops->map_lookup_batch(map, attr, uattr);
	} else {
		if (!map->ops->map_lookup_and_delete_batch) {
			err = -ENOTSUPP;
			goto err_put;
		}
