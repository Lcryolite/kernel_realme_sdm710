...
/* Raw tracepoints use their argument vector directly as the BPF context. */
static DEFINE_PER_CPU(struct pt_regs, bpf_raw_tp_regs);

BPF_CALL_5(bpf_perf_event_output_raw_tp,
	   struct bpf_raw_tracepoint_args *, args, struct bpf_map *, map,
	   u64, flags, void *, data, u64, size)
{
...
struct bpf_raw_event_map *bpf_get_raw_tracepoint(const char *name)
{
	...
	for (btp = __start__bpf_raw_tp; btp < __stop__bpf_raw_tp; btp++) {
...
{
	/* Only built-in tracepoint maps are registered in this backport. */
}
				...
				struct bpf_prog *prog)
{
	...
	return tracepoint_probe_register(btp->tp, btp->bpf_func, prog);
}
			 ...
			 struct bpf_prog *prog)
{
	...
	mutex_lock(&bpf_event_mutex);
	err = tracepoint_probe_unregister(btp->tp, btp->bpf_func, prog);
	mutex_unlock(&bpf_event_mutex);
	return err;
}
