...
out_dir=${OUT_DIR:-$source_repo/out-r021-usb-vbus-guard}
...
dwc3_driver="$source_repo/drivers/usb/dwc3/dwc3-msm.c"
...
grep -Fq 'skip gadget vbus connect' "$dwc3_driver" ||
	die "tracked dwc3 driver is missing the gadget vbus connect guard"
grep -Fq 'skip gadget vbus disconnect' "$dwc3_driver" ||
	die "tracked dwc3 driver is missing the gadget vbus disconnect guard"
...
summary="$out_dir/r021-gate-summary.txt"
{
	...
	printf 'candidate_kind=USB_GADGET_VBUS_NULL_GUARD\n'
	printf 'baseline_candidate=B14-M03-r020-ufs-ice-dt\n'
	printf 'functional_change=dwc3_otg_start_peripheral_skip_vbus_connect_disconnect_when_gadget_ops_not_ready\n'
	printf 'forbidden_functional_changes=DEFCONFIG,RAMDISK,UFS_DT,DISPLAY,FG_GEN3,CLANG\n'
	printf 'expected_defconfig_sha256=%s\n' "$expected_defconfig"
	sha256sum "$defconfig" "$ufs_dts" "$dwc3_driver" "$out_dir/.config" \
...
printf 'r021 USB gadget vbus guard build: PASS\n'
