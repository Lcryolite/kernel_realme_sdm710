...
/* Add a fault fixup for verifier-generated loads through BTF pointers. */
				 ...
				 struct jit_ctx *ctx, int dst_reg)
{
	...
	if (!ctx->image || BPF_MODE(insn->code) != BPF_PROBE_MEM)
		return 0;
	...
	if (WARN_ON_ONCE(offset < 0 || offset > BPF_FIXUP_OFFSET_MASK))
		return -ERANGE;
...
emit_cond_jmp:
	...
	case BPF_LDX | BPF_PROBE_MEM | BPF_DW:
		...
		err = add_exception_handler(insn, ctx, dst);
		if (err)
			return err;
		break;
...
struct bpf_prog *bpf_int_jit_compile(struct bpf_prog *prog)
{
	...
	/* Keep the sorted exception table adjacent to the JIT image. */
	prog_size = sizeof(u32) * ctx.idx;
