...
static inline u32 ipa3_debug_rule_ext_attrib_mask(
	const struct ipa_rule_attrib *attrib)
{
	return 0;
}

static inline u16 ipa3_debug_rule_payload_length(
	const struct ipa_rule_attrib *attrib)
{
	return 0;
}
		...
		enum ipa_ip_type ip)
{
	...
	if (ipa3_debug_rule_ext_attrib_mask(attrib) & IPA_FLT_EXT_NEXT_HDR)
		pr_err("next_hdr:%d ", attrib->u.v6.next_hdr);
	...
	if (ipa3_debug_rule_ext_attrib_mask(attrib) & IPA_FLT_EXT_MTU)
		pr_err("Payload Length:%d ",
			ipa3_debug_rule_payload_length(attrib));

	if (attrib->attrib_mask & IPA_FLT_MAC_ETHER_TYPE ||
		ipa3_debug_rule_ext_attrib_mask(attrib) &
