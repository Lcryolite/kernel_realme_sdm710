...
static int alloc_prefetch_info(
			struct ion_prefetch_regions_v2 __user *user_regions,
...
static int alloc_prefetch_legacy(struct ion_prefetch_regions __user *regions,
				 bool shrink, struct list_head *items)
{
	struct prefetch_info *info;
	...
	err = get_user(nr_sizes, &regions->nr_sizes);
	err |= get_user(user_sizes, &regions->sizes);
	err |= get_user(vmid, &regions->vmid);
	if (err)
		return -EFAULT;
...
#ifdef CONFIG_COMPAT
struct compat_prefetch_region {
	compat_uint_t vmid;
	compat_uptr_t sizes;
	compat_uint_t nr_sizes;
};

static int alloc_prefetch_compat(struct compat_prefetch_region __user *regions,
				 bool shrink, struct list_head *items)
{
	struct prefetch_info *info;
	compat_size_t __user *user_sizes;
	compat_uptr_t sizes;
	unsigned int nr_sizes, vmid, i;
	int err;

	err = get_user(nr_sizes, &regions->nr_sizes);
	err |= get_user(sizes, &regions->sizes);
	err |= get_user(vmid, &regions->vmid);
	if (err)
		return -EFAULT;
	user_sizes = compat_ptr(sizes);
	...
	for (i = 0; i < nr_sizes; i++) {
		...
		err = get_user(size, &user_sizes[i]);
		if (err)
			goto out_free;
...
static void free_prefetch_info(struct list_head *items)
{
	struct prefetch_info *info, *tmp;
...
static int queue_prefetch_info(struct ion_system_secure_heap *secure_heap,
			       bool shrink, struct list_head *items)
{
	unsigned long flags;
...
static int __ion_system_secure_heap_resize_v2(struct ion_heap *heap, void *ptr,
					      bool shrink)
{
	struct ion_system_secure_heap *secure_heap = container_of(heap,
						struct ion_system_secure_heap,
						heap);
	struct ion_prefetch_data_v2 *data = ptr;
	int i, ret = 0;
	LIST_HEAD(items);
	...
	for (i = 0; i < data->nr_regions; i++) {
		ret = alloc_prefetch_info(
			(struct ion_prefetch_regions_v2 *)data->regions + i,
			shrink, &items);
		if (ret)
			goto out_free;
	}
	return queue_prefetch_info(secure_heap, shrink, &items);
...
{
	return __ion_system_secure_heap_resize_v2(heap, ptr, false);
}

int ion_system_secure_heap_drain(struct ion_heap *heap, void *ptr)
{
	return __ion_system_secure_heap_resize_v2(heap, ptr, true);
}

static int __ion_system_secure_heap_resize_legacy(struct ion_heap *heap,
						  void *ptr, bool shrink)
{
	struct ion_system_secure_heap *secure_heap = container_of(heap,
						struct ion_system_secure_heap,
						heap);
	...
	for (i = 0; i < data->nr_regions; i++) {
		ret = alloc_prefetch_legacy(&data->regions[i], shrink, &items);
		if (ret)
			goto out_free;
	}
	return queue_prefetch_info(secure_heap, shrink, &items);
...
int ion_system_secure_heap_prefetch_legacy(struct ion_heap *heap, void *ptr)
{
	return __ion_system_secure_heap_resize_legacy(heap, ptr, false);
}

int ion_system_secure_heap_drain_legacy(struct ion_heap *heap, void *ptr)
{
	return __ion_system_secure_heap_resize_legacy(heap, ptr, true);
}

#ifdef CONFIG_COMPAT
static int __ion_system_secure_heap_resize_compat(struct ion_heap *heap,
						  void *ptr, bool shrink)
{
	struct ion_system_secure_heap *secure_heap = container_of(heap,
						struct ion_system_secure_heap,
						heap);
	struct ion_prefetch_data_compat *data = ptr;
	struct compat_prefetch_region __user *regions;
	int i, ret = 0;
	LIST_HEAD(items);
	...
	regions = compat_ptr(data->regions);
	for (i = 0; i < data->nr_regions; i++) {
		ret = alloc_prefetch_compat(&regions[i], shrink, &items);
		if (ret)
			goto out_free;
	}
	return queue_prefetch_info(secure_heap, shrink, &items);
...
int ion_system_secure_heap_prefetch_compat(struct ion_heap *heap, void *ptr)
{
	return __ion_system_secure_heap_resize_compat(heap, ptr, false);
}

int ion_system_secure_heap_drain_compat(struct ion_heap *heap, void *ptr)
{
	return __ion_system_secure_heap_resize_compat(heap, ptr, true);
}
#endif
