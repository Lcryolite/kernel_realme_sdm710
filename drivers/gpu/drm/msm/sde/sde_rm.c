				...
				struct sde_mdss_cfg *cat)
{
	...
	while (_sde_rm_get_hw_locked(rm, &iter_c)) {
		...
		if (intf_sel) {
			...
			pr_info("RMX1901-R010: cont-splash ctl=%d active single_flush=%d index_after=%d lm_cnt=%d pipe_cnt=%d\n",
				iter_c.blk->id - CTL_0,
				splash_display->single_flush_en, index,
				splash_display->lm_cnt, splash_display->pipe_cnt);
	...
	pr_info("RMX1901-R010: cont-splash final actual=%d expected=%u rc=%d\n",
		index, splash_data->num_splash_displays,
		index == splash_data->num_splash_displays ? 0 : -EINVAL);
